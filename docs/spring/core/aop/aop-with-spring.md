...
Within your Spring configurations, all aspect and advisor elements must be placed within an `` element (you can have more than one ``element in an application context configuration). An `` element can contain pointcut, advisor, and aspect elements (note these must be declared in that order).
...
![aop proxy call](https://docs.spring.io/spring/docs/current/spring-framework-reference/htmlsingle/images/aop-proxy-call.png)
...
If you are using XML based configuration you can specify the fully-qualified classname as the value of the `weaver-class` attribute on the `` element:
...
The book *AspectJ in Action, Second Edition* by Ramnivas Laddad (Manning, 2009) comes highly recommended; the focus of the book is on AspectJ, but a lot of general AOP themes are explored (in some depth).![img](https://docs.spring.io/spring/docs/current/spring-framework-reference/htmlsingle/images/tip.png)