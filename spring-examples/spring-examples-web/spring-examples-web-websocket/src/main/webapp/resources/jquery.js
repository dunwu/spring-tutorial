		...
		if (data === undefined && elem.nodeType === 1) {
			...
			if (typeof data === 'string') {
				try {
					...
					=== 'null' ? null : // Only convert to a number if it doesn't change the string
	...
	boolHook = {
		get: function (elem, name) {
			var // Use .prop to determine if this attribute is understood as boolean
				...
				getSetInput && getSetAttribute ? attr
					!= null : // oldIE fabricates an empty string for missing boolean attributes
	...
	jQuery.event = {
		...
		add: function (elem, types, handler, data, selector) {
			...
			var handleObjIn, eventHandle, tmp, events, t, handleObj, special, handlers, type, namespaces,
				origType, // Don't attach events to noData or text/comment nodes (but allow plain objects)
   ...
   */
				...
				superMatcher = function (seed, context, xml, results, expandContext) {
					var elem, j, matcher, setMatched = [], matchedCount = 0, i = '0', unmatched = seed && [],
						...
						elems = seed || byElement && Expr.find['TAG']('*', expandContext
							&& context.parentNode
							|| context), // Nested matchers should use non-integer dirruns
	...
	jQuery.fn.extend({
		...
		is: function (selector) {
			...
			=== 'string' ? // If this is a positional/relative selector, check membership in the returned set
	...
	function augmentWidthOrHeight(elem, name, extra, isBorderBox, styles) {
		var i = extra
			=== (isBorderBox ? 'border' : 'content') ? // If we already have the right measurement, avoid augmentation
		...
		ajax: function (url, options) {
			...
			var transport, // URL without anti-cache param
				...
				callbackContext = s.context
					|| s, // Context for global events is callbackContext if it is a DOM node or jQuery collection
   ...
   */
	...
	function ajaxConvert(s, response) {
		...
		var conv, conv2, current, tmp, converters = {},
			i = 0, // Work with a copy of dataTypes in case we need to modify it for conversion
