...
(function (window, undefined) {
	...
	var jQuery = (function () {
		...
		var jQuery = function (selector, context) {
			...
			// Useragent RegExp
			rwebkit = /(webkit)[ \/]([\w.]+)/, ropera = /(opera)(?:.*version)?[ \/]([\w.]+)/, rmsie = /(msie) ([\w.]+)/,
			...
			toString = Object.prototype.toString, hasOwn = Object.prototype.hasOwnProperty, push = Array.prototype.push,
			slice = Array.prototype.slice, trim = String.prototype.trim, indexOf = Array.prototype.indexOf,
		...
		jQuery.extend({
			...
			map: function (elems, callback, arg) {
				var value, key, ret = [], i = 0, length = elems.length, // jquery objects are treated as arrays
					isArray = elems instanceof jQuery || length !== undefined && typeof length === 'number' && ((length
						> 0
						&& elems[0]
	...
	jQuery.support = (function () {
		...
		var div = document.createElement('div'), documentElement = document.documentElement, all, a, select, opt, input,
			marginDiv, support, fragment, body, bodyStyle, tds, events, eventName, i, isSupported
		...
		bodyStyle = {
			...
			height: 0,
			border: 0,
			margin: 0, // Set background to avoid IE crashes when removing (#9028)
			background: 'none'
		}
		for (i in bodyStyle) {
	...
	function handleQueueMarkDefer(elem, type, src) {
		var deferDataKey = type + 'defer', queueDataKey = type + 'queue', markDataKey = type + 'mark',
			defer = jQuery.data(elem, deferDataKey, undefined, true)
		if (defer && (src === 'queue' || !jQuery.data(elem, queueDataKey, undefined, true)) && (src
			=== 'mark'
	...
	jQuery.fn.extend({
		...
		promise: function (type, object) {
			...
			type = type || 'fx'
			var defer = jQuery.Deferred(), elements = this, i = elements.length, count = 1, deferDataKey = type + 'defer',
	...
	jQuery.extend({
		...
		attr: function (elem, name, value, pass) {
			...
			if (!hooks) {
				...
				if (rboolean.test(name) && (typeof value
					=== 'boolean'
					|| value
					=== undefined
					|| value.toLowerCase()
		...
		attrHooks: {
			type: {
				...
				get: function (elem) {
					...
					&& attributeNode.specified ? parseInt(attributeNode.value, 10) : rfocusable.test(elem.nodeName)
	...
	var hasOwn = Object.prototype.hasOwnProperty, rnamespaces = /\.(.*)$/, rformElems = /^(?:textarea|input|select)$/i,
		rperiod = /\./g, rspaces = / /g, rescape = /[^\w\s.|`]/g, fcleanup = function (nm) {
   ...
   */
	jQuery.event = {
			...
			if (!eventHandle) {
				elemData.handle = eventHandle = function (e) {
					...
					return typeof jQuery !== 'undefined' && (!e
						...
						!== e.type) ? jQuery.event.handle.apply(eventHandle.elem, arguments) : undefined
				}
			}
		...
		remove: function (elem, types, handler, pos) {
			...
			var ret, type, fn, j, i = 0, all, namespaces, namespace, special, eventType, handleObj, origType,
				elemData = jQuery.hasData(elem) && jQuery._data(elem), events = elemData && elemData.events
		...
		trigger: function (event, data, elem, onlyHandlers) {
			...
			if (!event.isDefaultPrevented()) {
				...
				if ((!special._default || special._default.call(elem.ownerDocument, event) === false) && !(type
					=== 'click'
		...
		handle: function (event) {
			...
			var handlers = ((jQuery._data(this, 'events') || {})[event.type] || []).slice(0),
				run_all = !event.exclusive && !event.namespace, args = Array.prototype.slice.call(arguments, 0)
		...
		fix: function (event) {
			...
			if (event.pageX == null && event.clientX != null) {
				var eventDocument = event.target.ownerDocument || document, doc = eventDocument.documentElement,
					body = eventDocument.body

				event.pageX = event.clientX + (doc && doc.scrollLeft || body && body.scrollLeft || 0) - (doc
					&& doc.clientLeft
					|| body
					&& body.clientLeft
					|| 0)
		...
		jQuery.event.special.change = {
			filters: {
				...
				keydown: function (e) {
					...
					if ((e.keyCode === 13 && !jQuery.nodeName(elem, 'textarea')) || (e.keyCode === 32 && (type
						=== 'checkbox'
						|| type
						=== 'radio')) || type === 'select-multiple') {
						testChange.call(this, e)
					}
				},
	...
	jQuery.each(['live', 'die'], function (i, name) {
		jQuery.fn[name] = function (types, data, fn, origSelector /* Internal Use Only */) {
			var type, i = 0, match, namespaces, preType, selector = origSelector || this.selector,
	...
	function liveHandler(event) {
		var stop, maxLevel, related, match, handleObj, elem, j, i, l, data, close, namespace, ret, elems = [],
			selectors = [], events = jQuery._data(this, 'events')
   ...
   */
	(function () {
		...
		var chunker = /((?:\((?:\([^()]+\)|[^()]+)+\)|\[(?:\[[^\[\]]*\]|['"][^'"]*['"]|[^\[\]'"]+)+\]|\\.|[^ >+~,(\[\\]+)+|[>+~])(\s*,\s*)?((?:.|\r|\n)*)/g,
			done = 0, toString = Object.prototype.toString, hasDuplicate = false, baseHasDuplicate = true, rBackslash = /\\/g,
			rNonWord = /\W/;
		...
		var Sizzle = function (selector, context, results, seed) {
			...
			var m, set, checkSet, extra, ret, cur, pop, i, prune = true, contextXML = Sizzle.isXML(context), parts = [],
				soFar = selector
			...
			if (parts.length > 1 && origPOS.exec(selector)) {
				...
				if (context) {
					ret = seed ? {expr: parts.pop(), set: makeArray(seed)} : Sizzle.find(parts.pop(), parts.length
					=== 1
					&& (parts[0] === '~' || parts[0] === '+')
		...
		var Expr = Sizzle.selectors = {
			...
			relative: {
				'+': function (checkSet, part) {
					var isPartStr = typeof part === 'string', isTag = isPartStr && !rNonWord.test(part),
			...
			filters: {
				...
				ATTR: function (elem, match) {
					var name = match[1], result = Expr.attrHandle[name] ? Expr.attrHandle[name](elem) : elem[name]
					!= null ? elem[name] : elem.getAttribute(name), value = result + '', type = match[2], check = match[4]
					...
					=== '!=' ? value !== check : type === '^=' ? value.indexOf(check) === 0 : type
					=== '$=' ? value.substr(value.length - check.length) === check : type === '|=' ? value
						=== check
		...
		(function () {
			...
			if (div.firstChild
				&& typeof div.firstChild.getAttribute
				!== 'undefined'
					...
					if (!seed && !Sizzle.isXML(context)) {
						...
						if (context.nodeType === 9) {
							...
							var oldContext = context, old = context.getAttribute('id'), nid = old || id,
								hasParent = context.parentNode, relativeHierarchySelector = /^\s*[+~]/.test(query)
	...
	var runtil = /Until$/,
		rparentsprev = /^(?:parents|prevUntil|prevAll)/, // Note: This RegExp should be improved, or likely pulled from
																										 // Sizzle
		rmultiselector = /,/, isSimple = /^.[^:#\[\.,]*$/, slice = Array.prototype.slice,
		POS = jQuery.expr.match.POS, // methods guaranteed to produce a unique set when starting from a unique set
		guaranteedUnique = {
	...
	jQuery.fn.extend({
		...
		is: function (selector) {
			return !!selector && (typeof selector === 'string' ? jQuery.filter(selector, this).length
	...
	var rinlinejQuery = / jQuery\d+="(?:\d+|null)"/g, rleadingWhitespace = /^\s+/,
		rxhtmlTag = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig, rtagName = /<([\w:]+)/,
		rtbody = /<tbody/i, rhtml = /<|&#?\w+;/,
		rnocache = /<(?:script|object|embed|option|style)/i, // checked="checked" or checked
		rchecked = /checked\s*(?:[^=]|=\s*.checked.)/i, rscriptType = /\/(java|ecma)script/i,
		rcleanScript = /^\s*<!(?:\[CDATA\[|\-\-)/, wrapMap = {
				...
				for (var type in events) {
					for (var i = 0, l = events[type].length; i < l; i++) {
						jQuery.event.add(dest, type
							...
							+ events[type][i].namespace, events[type][i], events[type][i].data)
					}
				}
			}
		}
			...
			for (var i = 0, elem; (elem = elems[i]) != null; i++) {
				...
				if (typeof elem === 'string') {
					if (!rhtml.test(elem)) {
						...
						var tag = (rtagName.exec(elem) || ['', ''])[1].toLowerCase(), wrap = wrapMap[tag] || wrapMap._default,
	...
	var ralpha = /alpha\([^)]*\)/i, ropacity = /opacity=([^)]*)/, rdashAlpha = /-([a-z])/ig, // fixed for IE9, see #8346
		rupper = /([A-Z]|^ms)/g, rnumpx = /^-?\d+(?:px)?$/i, rnum = /^-?\d/, rrelNum = /^[+\-]=/,
		rrelNumFilter = /[^+\-\.\de]+/g,

		cssShow = {position: 'absolute', visibility: 'hidden', display: 'block'}, cssWidth = ['Left', 'Right'],
		cssHeight = ['Top', 'Bottom'], curCSS,
	...
	if (document.documentElement.currentStyle) {
		currentStyle = function (elem, name) {
			var left, ret = elem.currentStyle && elem.currentStyle[name],
				rsLeft = elem.runtimeStyle && elem.runtimeStyle[name], style = elem.style
	...
	var r20 = /%20/g, rbracket = /\[\]$/, rCRLF = /\r?\n/g, rhash = /#.*$/, rheaders = /^(.*?):[ \t]*([^\r\n]*)\r?$/mg, // IE
																																																										...
																																																										// at
																																																										// EOL
		rinput = /^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i, // #7653,
																																																									// #8125,
																																																									// #8152:
																																																									// local
																																																									// protocol
																																																									// detection
		rlocalProtocol = /^(?:about|app|app\-storage|.+\-extension|file|widget):$/, rnoContent = /^(?:GET|HEAD)$/,
		rprotocol = /^\/\//, rquery = /\?/, rscript = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,
		rselectTextarea = /^(?:select|textarea)/i, rspacesAjax = /\s+/, rts = /([?&])_=[^&]*/,
		rurl = /^([\w\+\.\-]+:)(?:\/\/([^\/?#:]*)(?::(\d+))?)?/,
	...
	function addToPrefiltersOrTransports(structure) {
		...
		return function (dataTypeExpression, func) {
			...
			if (jQuery.isFunction(func)) {
				var dataTypes = dataTypeExpression.toLowerCase().split(rspacesAjax), i = 0, length = dataTypes.length, dataType,
					list, placeBefore
	...
	function inspectPrefiltersOrTransports(structure, options, originalOptions, jqXHR, dataType /* internal */, inspected /* internal */) {
		...
		var list = structure[dataType], i = 0, length = list ? list.length : 0, executeOnly = (structure === prefilters),
			selection
	...
	jQuery.fn.extend({
		load: function (url, params, callback) {
			...
			jQuery.ajax({
				url: url, type: type, dataType: 'html', data: params, // Complete callback (responseText is used internally)
	...
	jQuery.each(['get', 'post'], function (i, method) {
		jQuery[method] = function (url, data, callback, type) {
			...
			return jQuery.ajax({
				type: method, url: url, data: data, success: callback, dataType: type
			})
		}
	})
		...
		ajax: function (url, options) {
			...
			var // Create the final options object
				...
				globalEventContext = callbackContext !== s && (callbackContext.nodeType
					|| callbackContext
					instanceof jQuery) ? jQuery(callbackContext) : jQuery.event, // Deferreds
			...
			function done(status, statusText, responses, headers) {
				...
				if (fireGlobals) {
					globalEventContext.trigger('ajax'
						+ (isSuccess ? 'Success' : 'Error'), [jqXHR, s, isSuccess ? success : error])
				}
			...
			if (s.crossDomain == null) {
				...
				s.crossDomain = !!(parts && (parts[1] != ajaxLocParts[1] || parts[2] != ajaxLocParts[2] || (parts[3]
						|| (parts[1] === 'http:' ? 80 : 443)) != (ajaxLocParts[3] || (ajaxLocParts[1] === 'http:' ? 80 : 443))))
			}
   ...
   */
	function ajaxHandleResponses(s, jqXHR, responses) {
		...
		var contents = s.contents, dataTypes = s.dataTypes, responseFields = s.responseFields, ct, type, finalDataType,
			firstDataType
	...
	function ajaxConvert(s, response) {
		...
		var dataTypes = s.dataTypes, converters = {}, i, key, length = dataTypes.length,
			tmp, // Current and previous dataTypes
	...
	jQuery.ajaxPrefilter('json jsonp', function (s, originalSettings, jqXHR) {
		...
		if (s.dataTypes[0] === 'jsonp' || s.jsonp !== false && (jsre.test(s.url) || inspectData && jsre.test(s.data))) {
			...
			var responseContainer,
				...
				previous = window[jsonpCallback], url = s.url, data = s.data, replace = '$1' + jsonpCallback + '$2'
	...
	(function (xhr) {
		jQuery.extend(jQuery.support, {
			ajax: !!xhr, cors: !!xhr && ('withCredentials' in xhr)
		})
	...
	var elemdisplay = {}, iframe, iframeDoc, rfxtypes = /^(?:toggle|show|hide)$/,
		rfxnum = /^([+\-]=)?([\d+.\-]+)([a-z%]*)$/i, timerId, fxAttrs = [// height animations
	...
	jQuery.fn.extend({
		...
		animate: function (prop, speed, easing, callback) {
			...
			return this[optall.queue === false ? 'each' : 'queue'](function () {
				...
				var opt = jQuery.extend({}, optall), isElement = this.nodeType === 1,
					hidden = isElement && jQuery(this).is(':hidden'), name, val, p, display, e, parts, start, end, unit
	...
	jQuery.extend({
		speed: function (speed, easing, fn) {
			...
			opt.duration = jQuery.fx.off ? 0 : typeof opt.duration === 'number' ? opt.duration : opt.duration
	...
	if ('getBoundingClientRect' in document.documentElement) {
		jQuery.fn.offset = function (options) {
			...
			var body = doc.body, win = getWindow(doc), clientTop = docElem.clientTop || body.clientTop || 0,
		...
		jQuery.fn.offset = function (options) {
			...
			var computedStyle, offsetParent = elem.offsetParent, prevOffsetParent = elem, doc = elem.ownerDocument,
				docElem = doc.documentElement, body = doc.body, defaultView = doc.defaultView,
	...
	jQuery.offset = {
		...
		setOffset: function (elem, options, i) {
			...
			var curElem = jQuery(elem), curOffset = curElem.offset(), curCSSTop = jQuery.css(elem, 'top'),
				...
				&& jQuery.inArray('auto', [curCSSTop, curCSSLeft])
	...
	jQuery.each(['Left', 'Top'], function (i, name) {
		...
		jQuery.fn[method] = function (val) {
			...
			if (val === undefined) {
				...
				return win ? ('pageXOffset' in win) ? win[i ? 'pageYOffset' : 'pageXOffset'] : jQuery.support.boxModel
	...
	jQuery.each(['Height', 'Width'], function (i, name) {
		...
		jQuery.fn[type] = function (size) {
			...
			if (jQuery.isWindow(elem)) {
				...
				return elem.document.compatMode
					=== 'CSS1Compat'
					&& docElemProp
				...
				return Math.max(elem.documentElement['client' + name], elem.body['scroll' + name], elem.documentElement['scroll'
				+ name], elem.body['offset' + name], elem.documentElement['offset' + name])
