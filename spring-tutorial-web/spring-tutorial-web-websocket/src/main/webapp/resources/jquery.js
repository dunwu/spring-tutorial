...
(function (window, undefined) {
	...
	var // A central reference to the root jQuery(document)
		...
		core_concat = core_deletedIds.concat, core_push = core_deletedIds.push, core_slice = core_deletedIds.slice,
		core_indexOf = core_deletedIds.indexOf, core_toString = class2type.toString,
		core_hasOwn = class2type.hasOwnProperty, core_trim = core_version.trim,
		...
		// JSON RegExp
		rvalidchars = /^[\],:{}\s]*$/, rvalidbraces = /(?:^|:|,)(?:\s*\[)+/g,
	...
	function isArraylike(obj) {
		...
		return type === 'array' || type !== 'function' && (length
			=== 0
			|| typeof length
			=== 'number'
			&& length
   ...
   */
	jQuery.event = {
		...
		add: function (elem, types, handler, data, selector) {
			...
			var handleObjIn, eventHandle, tmp, events, t, handleObj, special, handlers, type, namespaces, origType, // Don't attach
																																																				// events to
																																																				// noData or
																																																				// text/comment
																																																				// nodes (but
		...
		remove: function (elem, types, handler, selector, mappedTypes) {
			...
			while (t--) {
				...
				while (j--) {
					...
					if ((mappedTypes || origType === handleObj.origType) && (!handler || handler.guid === handleObj.guid) && (!tmp
						|| tmp.test(handleObj.namespace)) && (!selector
						|| selector
						=== handleObj.selector
						|| selector
						=== '**'
		...
		trigger: function (event, data, elem, onlyHandlers) {
			...
			var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [elem || document], type = event.type || event,
   ...
   */
	(function (window, undefined) {
		...
		var i, cachedruns, Expr, getText, isXML, compile, hasDuplicate, outermostContext,
			...
			// Array methods
			arr = [], pop = arr.pop, push = arr.push, slice = arr.slice, // Use a stripped-down indexOf if we can't use a
																																	 // native one
           ...
           */
			...
			filter: {
				...
				'ATTR': function (name, operator, check) {
					return function (elem) {
						...
						=== '^=' ? check && result.indexOf(check) === 0 : operator === '*=' ? check
							...
							> -1 : operator === '$=' ? check && result.substr(result.length - check.length) === check : operator
						=== '~=' ? (' ' + result + ' ').indexOf(check) > -1 : operator === '|=' ? result
							=== check
				...
				'PSEUDO': function (pseudo, argument) {
					...
					var args,
						fn = Expr.pseudos[pseudo] || Expr.setFilters[pseudo.toLowerCase()] || Sizzle.error('unsupported pseudo: '
							+ pseudo)
			...
			pseudos: {
				...
				'text': function (elem) {
					...
					return elem.nodeName.toLowerCase() === 'input' && elem.type === 'text' && ((attr = elem.getAttribute('type'))
						== null
						|| attr.toLowerCase()
						=== elem.type)
				},
			...
			for (; i < len; i++) {
				if ((matcher = Expr.relative[tokens[i].type])) {
					...
					if (matcher[expando]) {
						...
						return setMatcher(i > 1 && elementMatcher(matchers), i > 1 && toSelector(tokens.slice(0, i
							- 1)).replace(rtrim, '$1'), matcher, i < j && matcherFromTokens(tokens.slice(i, j)), j
							< len
			...
			var matcherCachedRuns = 0, bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0,
				superMatcher = function (seed, context, xml, results, expandContext) {
					var elem, j, matcher, setMatched = [], matchedCount = 0, i = '0', unmatched = seed && [],
						outermost = expandContext != null, contextBackup = outermostContext, // We must always have either seed
																																								 // elements or context
						elems = seed || byElement && Expr.find['TAG']('*', expandContext && context.parentNode || context), // Nested
																																																								// matchers
																																																								// should
																																																								// use
																																																								// non-integer
	...
	var runtil = /Until$/, rparentsprev = /^(?:parents|prev(?:Until|All))/, isSimple = /^.[^:#\[\.,]*$/,
		rneedsContext = jQuery.expr.match.needsContext, // methods guaranteed to produce a unique set when starting from a
																										// unique set
		guaranteedUnique = {
		...
		is: function (selector) {
			return !!selector && (typeof selector === 'string' ? // If this is a positional/relative selector, check membership in the returned set
				...
				rneedsContext.test(selector) ? jQuery(selector, this.context).index(this[0])
					>= 0 : jQuery.filter(selector, this).length > 0 : this.filter(selector).length > 0)
		},

		closest: function (selectors, context) {
			var cur, i = 0, l = this.length, ret = [],
				pos = rneedsContext.test(selectors) || typeof selectors !== 'string' ? jQuery(selectors, context
					|| this.context) : 0
	...
	var nodeNames = 'abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|'
		...
		rleadingWhitespace = /^\s+/, rxhtmlTag = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi,
		rtagName = /<([\w:]+)/, rtbody = /<tbody/i, rhtml = /<|&#?\w+;/, rnoInnerhtml = /<(?:script|style|link)/i,
			...
			if (l) {
				...
				if (first) {
					...
					for (; i < l; i++) {
						...
						callback.call(table
						&& jQuery.nodeName(this[i], 'table') ? findOrAppend(this[i], 'tbody') : this[i], node, i)
					}
						...
						for (i = 0; i < hasScripts; i++) {
							...
							if (rscriptType.test(node.type || '')
								...
								if (node.src) {
									...
									jQuery.ajax({
										url: node.src, type: 'GET', dataType: 'script', async: false, global: false, 'throws': true
									})
								} else {
	...
	var curCSS, getStyles, iframe, ralpha = /alpha\([^)]*\)/i, ropacity = /opacity\s*=\s*([^)]*)/,
		rposition = /^(top|right|bottom|left)$/, // swappable if display is none or starts with table except "table",
																						 // "table-cell", or "table-caption"
		// see here for display values: https://developer.mozilla.org/en-US/docs/CSS/display
		rdisplayswap = /^(none|table(?!-c[ea]).+)/, rmargin = /^margin/,
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		return (val + augmentWidthOrHeight(elem, name, extra
			|| (isBorderBox ? 'border' : 'content'), valueIsBorderBox, styles)) + 'px'
	}
		...
		jQuery.cssHooks[name] = {
			get: function (elem, computed, extra) {
				if (computed) {
					...
					=== 0
					&& rdisplayswap.test(jQuery.css(elem, 'display')) ? jQuery.swap(elem, cssShow, function () {
	...
	var r20 = /%20/g, rbracket = /\[\]$/, rCRLF = /\r?\n/g, rsubmitterTypes = /^(?:submit|button|image|reset)$/i,
       ...
       */
		...
		ajax: function (url, options) {
			...
			var transport, // URL without anti-cache param
				...
				callbackContext = s.context || s, // Context for global events is callbackContext if it is a DOM node or jQuery
																					// collection
				globalEventContext = s.context && (callbackContext.nodeType
					|| callbackContext.jquery) ? jQuery(callbackContext) : jQuery.event, // Deferreds
   ...
   */
	...
	function ajaxConvert(s, response) {
		...
		var conv, conv2, current, tmp, converters = {}, i = 0, // Work with a copy of dataTypes in case we need to modify it
																											 // for conversion
	...
	var xhrCallbacks, xhrSupported, xhrId = 0, // #5280: Internet Explorer will keep connections alive if we don't abort on
																					 // unload
		...
		var result, stopped, index = 0, length = animationPrefilters.length,
			deferred = jQuery.Deferred().always(function () {
				...
				var currentTime = fxNow || createFxNow(),
					remaining = Math.max(0, animation.startTime + animation.duration - currentTime), // archaic crash bug won't
																																													 // allow us to use 1 - ( 0.5
																																													 // || 0 ) (#12497)
	...
	function defaultPrefilter(elem, props, opts) {
		...
		var index, prop, value, length, dataShow, toggle, tween, hooks, oldfire, anim = this, style = elem.style, orig = {},
			...
			jQuery.fn[funcName] = function (margin, value) {
				...
				return jQuery.access(this, function (elem, type, value) {
					...
					if (elem.nodeType === 9) {
						...
						return Math.max(elem.body['scroll' + name], doc['scroll' + name], elem.body['offset' + name], doc['offset'
